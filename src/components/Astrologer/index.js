...
class Astrologer extends Component{
	...
	handleDeleteRow(rowId){
		...
		if (r == true) {
			...
			config.get('/api/astro/deleteAstro?id='+rowId,{
										...
										<table className="datatable table table-hover table-center mb-0">	
											...
											{astrolist.map((rowData, index) => (
												<tr key={index+1}>
													...
													<td>Active</td>
													<td>
														<Link to={''} className="btn btn-xs btn-primary">Edit</Link>
															&nbsp;&nbsp;
														<Link className="btn btn-xs btn-danger" onClick={i => this.handleDeleteRow(rowData.id)}>Trash</Link>
													</td>
												</tr>
											))}
											 </tbody>	
